...
<table class="table table-bordered table-striped {{ count($games) > 0 ? 'datatable' : '' }}">
    <thead>
        <tr>
            <th>@lang('global.games.fields.team1')</th>
                        <th>@lang('global.games.fields.team2')</th>
                        <th>@lang('global.games.fields.start-time')</th>
                        <th>@lang('global.games.fields.result1')</th>
                        <th>@lang('global.games.fields.result2')</th>
                        @if( request('show_deleted') == 1 )
                        <th>&nbsp;</th>
                        @else
                        <th>&nbsp;</th>
            ...
            @foreach ($games as $game)
                <tr data-entry-id="{{ $game->id }}">
                    <td field-key='team1'>{{ $game->team1->name ?? '' }}</td>
                                ...
                                <td>
                                    @can('game_view')
                                    <a href="{{ route('admin.games.show',[$game->id]) }}" class="btn btn-xs btn-primary">@lang('global.app_view')</a>
                                    @endcan
                                    @can('game_edit')
                                    <a href="{{ route('admin.games.edit',[$game->id]) }}" class="btn btn-xs btn-info">@lang('global.app_edit')</a>
                                    @endcan
                                    @can('game_delete')
{!! Form::open(array(
                                        'style' => 'display: inline-block;',
